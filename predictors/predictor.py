...
class Predictor():
    def __init__(self, config,  checkpoint_path='./snapshots/checkpoint_best.pth.tar'):
        ...
        self.categories_dict = {"background": 0, "hair": 1}
        ...
        self.base_size= self.config['image']['base_size']
    ...
    def inference_on_test_set(self):
        ...
        for i, sample in enumerate(tbar):
            ...
            criterion = torch.nn.BCELoss().cuda() #added by me
            target=target[:,:2,:,:] #added by me
            output = torch.nn.Sigmoid()(output) #added by me
            loss = criterion(output, target) #added by me
            test_loss += loss.item()
    ...
    def segment_image(self, filename):
        ...
        sample = np_loader.preprocess(sample, self.config['image']['crop_size'], self.config['dataset']['mean'], self.config['dataset']['std'])
